...
export function useReplayOverlay(viewer: Cesium.Viewer | null) {
    ...
    // Replay-mode TrailBatcher: hits POST /api/trajectories on seek/layer change
    // and renders a 'replay' shard of trails up to currentTime. Disposes on
    // mode exit.
    useEffect(() => {
        ...
        let cancelled = false;
        const reqVersion = replayCancelVersionRef.current;
        let abort = new AbortController();
        let lastFetchEndMs = 0;
        let fetchInFlight = false;
        const REFETCH_AFTER_MS = 30_000; // playback-time delta triggering refresh

        const fetchTrails = async (trigger: 'initial' | 'tick') => {
            if (cancelled || replayCancelVersionRef.current !== reqVersion) return;
        ...
        // Roll the replay trail window forward during ordinary playback. The
        // effect deps only invalidate on seek / layer change / mode switch,
        // so without this tick the trail is frozen while viewer.clock drifts.
